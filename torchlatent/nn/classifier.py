...
class BiaffineClassifier(nn.Module):
    def __init__(self, bias: bool = False, *,
        ...
        self.weight0 = nn.Parameter(torch.empty((out_features, in_features1, in_features2)))
        self.weight1 = nn.Parameter(torch.empty((out_features, in_features1)))
        self.weight2 = nn.Parameter(torch.empty((out_features, in_features2)))
        self.bias = nn.Parameter(torch.empty((out_features,))) if bias else 0
    ...
    def forward(self, tensor1: Tensor, tensor2: Tensor) -> Tensor:
        tensor0 = torch.einsum('zxy,...x,...y->...z', self.weight0, tensor1, tensor2)
        tensor1 = torch.einsum('zx,...x->...z', self.weight1, tensor1)
        tensor2 = torch.einsum('zy,...y->...z', self.weight2, tensor2)
