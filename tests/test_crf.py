...
@given(
    ...
    rua_logits=st.sampled_from(Z.__args__),
    rua_targets=st.sampled_from(Z.__args__),
)
def test_crf_scores(token_sizes, num_targets, rua_logits, rua_targets):
    ...
    expected_logits = L.new(inputs)
    expected_tags = L.new(targets)

    expected = expected_crf._compute_score(
        expected_logits.data.transpose(0, 1),
        expected_tags.data.transpose(0, 1),
        expected_logits.bmask().transpose(0, 1),
    )

    actual = crf_scores(
        logits=rua_logits.new(inputs),
        targets=rua_targets.new(targets),
...
def test_crf_argmax(token_sizes, num_targets, rua_logits):
    ...
    actual = actual_crf(rua_logits.new(inputs)).argmax.cat()
